...
*** Anonymity
    A social choice function is /anonymous/ if the names of the agents do not matter, i.e. if a permutation of preferences across agents does not change the social preference.
...
*** Positive responsiveness
    A social welfare function is /positively responsive/ if the following holds: if one alternative, say $x$, is weakly socially preferred although $y\succ_i x$ for some $i\in\{1,\dots,N\}$, then $x$ is strictly socially preferred if we change $i$'s preferences (without changing anyone else's preferences).
...
*** Proof sketch ("only if" for even $N$)
    ...
    - Neutrality: if $N/2$ people prefer $x$ over $y$, then $x\approx_S y$.
...
** Examples: social welfare function
   ...
   - Borda Count:
     ...
     - order alternatives according to points
...
**  Borda and Olympic Ice Skating  competition II
   - say, first rank gives 3 points, second 2 and third 1
...
- imagine alternatives are ordered on a real line $x_1<x_2<\dots <x_K$   
- assumptions:
...
*** Proof
    ...
    # - As $N$ is odd and preferences are strict, pairwise majority voting yields a strict winner between any two alternatives.
    #  - Hence, the social preference ordering resulting from pairwise majority voting is complete and strict.
    # - Say $x_m\succ_S x_k$ and $x_k\succ_S x_l$
...
*** Proof
    - let $W$ be strictly increasing and $x$ Pareto dominate $y$
    #  - then $u_i(x)\geq u_i(y)$ for all $i$ and strictly so for at least one agent
    #  - hence the utility vector u(x)>u(y)
    #  - hence $W(u_1(x),u_2(x),\dots,u_N(x))> W(u_1(y),u_2(y),\dots,u_N(y))$ as $W$ is strictly increasing
    #  - hence $x$ is socially preferred to $y$
 \vspace*{2cm}

** Cardinal utility V: Rawlsian welfare
   $$W_{Rawls}(u_1,\dots,u_N)=\min[u_1,\dots,u_N]$$

  - $W_{Rawls}$ is strictly increasing $\Rightarrow$ satisfies Pareto criterion
...
*** :B_ignoreheading:BMCOL:
    ...
    :BEAMER_col: 0.4
    :END:
- $\approx$ Rawlsian welfare is equivalent to Pareto criterion  + Hammond equity 
*** Proof      
